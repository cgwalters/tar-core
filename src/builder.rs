...
/// Write a u64 value as octal ASCII to a fixed-size header field.
///
/// The value is formatted as octal with leading zeros and a null terminator.
/// For example, mode 0o644 in an 8-byte field becomes `b"0000644\0"`.
///
/// The const generic `N` carries the field size from the zerocopy header
/// struct, making the maximum representable value visible at compile time.
///
/// # Errors
///
...
impl HeaderBuilder {
    ...
    /// # Errors
    ///
    /// Returns an error if the mode doesn't fit in the 8-byte octal field
    /// (max value is 0o7777777 / 2097151).
    pub fn mode(&mut self, mode: u32) -> Result<&mut Self> {
        write_octal(&mut self.fields_mut().mode, u64::from(mode))?;
        Ok(self)
    }
    ...
    pub fn uid(&mut self, uid: u64) -> Result<&mut Self> {
        write_octal(&mut self.fields_mut().uid, uid)?;
        Ok(self)
    }
    ...
    pub fn gid(&mut self, gid: u64) -> Result<&mut Self> {
        write_octal(&mut self.fields_mut().gid, gid)?;
        Ok(self)
    }
    ...
    pub fn size(&mut self, size: u64) -> Result<&mut Self> {
        write_octal(&mut self.fields_mut().size, size)?;
        Ok(self)
    }
    ...
    pub fn mtime(&mut self, mtime: u64) -> Result<&mut Self> {
        write_octal(&mut self.fields_mut().mtime, mtime)?;
        Ok(self)
    }
    ...
    pub fn device(&mut self, major: u32, minor: u32) -> Result<&mut Self> {
        write_octal(&mut self.fields_mut().devmajor, u64::from(major))?;
        write_octal(&mut self.fields_mut().devminor, u64::from(minor))?;
        Ok(self)
    }
...
impl EntryBuilder {
    ...
    /// # Panics
    ///
    /// Panics if the mode exceeds the maximum value for the 7-digit octal
    /// field (0o7777777 / 2097151). Standard Unix permission modes (up to
    /// 0o7777) always fit.
    pub fn mode(&mut self, mode: u32) -> &mut Self {
        self.header
            .mode(mode)
            .expect("mode value exceeds tar header field capacity");
        self
    }
    ...
    pub fn add_pax(&mut self, key: &str, value: &[u8]) -> &mut Self {
        self.pax.get_or_insert_with(PaxBuilder::new).add(key, value);
        self
    }
