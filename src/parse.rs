...
pub struct Limits {
    ...
    ///
    /// Default: 1 MiB (1,048,576 bytes), matching Go's `archive/tar`.
...
impl Default for Limits {
    fn default() -> Self {
        Self {
            ...
            max_gnu_long_size: 1024 * 1024, // 1 MiB, matching Go's archive/tar
...
impl Limits {
    ...
    pub fn strict() -> Self {
        Self {
            ...
            max_gnu_long_size: 64 * 1024, // 64 KiB, matching max_pax_size
...
mod tests {
    ...
    fn test_default_limits() {
        ...
        assert_eq!(limits.max_gnu_long_size, 1024 * 1024);
    ...
    fn test_strict_limits() {
        ...
        assert_eq!(limits.max_gnu_long_size, 64 * 1024);
    }
    ...
    fn test_parser_with_real_header() {
        ...
        let header = make_header(b"test.txt", 0, b'0');

        // Build archive: header + two zero blocks
        let mut data = vec![0u8; HEADER_SIZE + 1024];
    ...
    fn test_parser_entry_with_content() {
        ...
        let header = make_header(b"test.txt", 5, b'0');

        // Build archive: header + content block + 2 zero blocks
        let mut data = vec![0u8; HEADER_SIZE + 512 + 1024];
        data[..HEADER_SIZE].copy_from_slice(&header);
    ...
    fn make_header(name: &[u8], size: u64, typeflag: u8) -> [u8; HEADER_SIZE] {
        ...
        header.set_path(&name[..name.len().min(100)]).unwrap();
        header.set_mode(0o644).unwrap();
        header.set_uid(1000).unwrap();
        header.set_gid(1000).unwrap();
        header.set_size(size).unwrap();
        header.set_mtime(0o14712345670).unwrap();
        header.set_entry_type(EntryType::from_byte(typeflag));
        header.set_checksum();
        *header.as_bytes()
    }
    ...
    fn make_link_header(name: &[u8], link_target: &[u8], typeflag: u8) -> [u8; HEADER_SIZE] {
        let mut bytes = make_header(name, 0, typeflag);
        let header = Header::mut_from_bytes(&mut bytes).unwrap();
        header
            .set_link_name(&link_target[..link_target.len().min(100)])
            .unwrap();
        header.set_checksum();
        bytes
    }
    ...
    fn make_gnu_ext_sparse(entries: &[(u64, u64)], is_extended: bool) -> [u8; HEADER_SIZE] {
        ...
        for (i, &(offset, length)) in entries.iter().enumerate() {
            ext.sparse[i].set(&SparseEntry { offset, length });
        }
        ext.set_is_extended(is_extended);
        zerocopy::IntoBytes::as_bytes(&ext).try_into().unwrap()
    }
