...
impl Header {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<&Header> {
        ...
        Header::ref_from_bytes(&bytes[..HEADER_SIZE])
    ...
    #[must_use]
    pub fn from_bytes_exact(bytes: &[u8; HEADER_SIZE]) -> &Header {
    ...
    pub fn set_size(&mut self, size: u64) {
        let _ = encode_numeric(&mut self.bytes[124..136], size);
    }
    ...
    pub fn set_mode(&mut self, mode: u32) {
        let _ = encode_octal(&mut self.bytes[100..108], u64::from(mode));
    }
    ...
    pub fn set_uid(&mut self, uid: u64) {
        let _ = encode_numeric(&mut self.bytes[108..116], uid);
    }
    ...
    pub fn set_gid(&mut self, gid: u64) {
        let _ = encode_numeric(&mut self.bytes[116..124], gid);
    }
    ...
    pub fn set_mtime(&mut self, mtime: u64) {
        let _ = encode_numeric(&mut self.bytes[136..148], mtime);
    }
    ...
    pub fn set_device(&mut self, major: u32, minor: u32) {
        let _ = encode_octal(&mut self.bytes[329..337], u64::from(major));
        let _ = encode_octal(&mut self.bytes[337..345], u64::from(minor));
    }
}
...
mod tests {
    ...
    fn test_encode_numeric_roundtrip() {
        // (field_size, value, expect_base256)
        let cases: &[(usize, u64, bool)] = &[
            // 12-byte field: octal range
            (12, 0, false),
            (12, 0o644, false),
            (12, 0o77777777777, false),
            // 12-byte field: base-256 (>= 8GB threshold)
            (12, 8_589_934_592, true),
            (12, 0x1234_5678_90AB_CDEF, true),
            // 8-byte field (uid/gid): octal range
            (8, 0, false),
            (8, 2_097_151, false), // just below threshold
            // 8-byte field: base-256 (>= 2^21 threshold)
            (8, 2_097_152, true),
        ];
        for &(size, value, expect_b256) in cases {
            let mut field = vec![0u8; size];
            encode_numeric(&mut field, value).unwrap();
            assert_eq!(
                field[0] & 0x80 != 0,
                expect_b256,
                "base256 flag for {value} in {size}-byte field"
            );
            assert_eq!(
                parse_numeric(&field).unwrap(),
                value,
                "roundtrip {value} in {size}-byte field"
            );
        }
    }
    ...
    fn test_header_setters() {
        ...
        // Numeric field setters: (set, get, value)
        let numeric_cases: &[(fn(&mut Header, u64), fn(&Header) -> Result<u64>, u64)] = &[
            ...
            // Large values requiring base-256
            (|h, v| h.set_uid(v), |h| h.uid(), 0xFFFF_FFFF),
            (|h, v| h.set_size(v), |h| h.entry_size(), 10_000_000_000),
        ];
        for (set, get, value) in numeric_cases {
            set(&mut header, *value);
