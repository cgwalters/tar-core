...
//! with any I/O model. The [`stream::Parser`] has no trait bounds on readers—it
...
//! use tar_core::stream::{TarStreamParser, Limits};
//!
...
pub use builder::{
    blocks_for_size, pad_to_block_boundary, EntryBuilder, ExtensionMode, HeaderBuilder, PaxBuilder,
    LINKNAME_MAX_LEN, NAME_MAX_LEN,
};
...
impl GnuHeader {
    ...
    pub fn set_atime(&mut self, atime: u64) {
        let _ = encode_numeric(&mut self.atime, atime);
    }
    ...
    pub fn set_ctime(&mut self, ctime: u64) {
        let _ = encode_numeric(&mut self.ctime, ctime);
    }
    ...
    pub fn set_real_size(&mut self, size: u64) {
        let _ = encode_numeric(&mut self.realsize, size);
    }
...
impl Header {
    ...
    pub fn as_old_mut(&mut self) -> &mut OldHeader {
        OldHeader::mut_from_bytes(&mut self.raw.bytes).expect("size is correct")
    }

    /// View this header as a mutable UStar header.
    #[must_use]
    pub fn as_ustar_mut(&mut self) -> &mut UstarHeader {
        UstarHeader::mut_from_bytes(&mut self.raw.bytes).expect("size is correct")
    }

    /// View this header as a mutable GNU header.
    #[must_use]
    pub fn as_gnu_mut(&mut self) -> &mut GnuHeader {
        GnuHeader::mut_from_bytes(&mut self.raw.bytes).expect("size is correct")
    }
    ...
    pub fn set_size(&mut self, size: u64) {
        let _ = encode_numeric(&mut self.raw.bytes[124..136], size);
    }
    ...
    pub fn set_mode(&mut self, mode: u32) {
        let _ = encode_octal(&mut self.raw.bytes[100..108], u64::from(mode));
    }
    ...
    pub fn set_uid(&mut self, uid: u64) {
        let _ = encode_numeric(&mut self.raw.bytes[108..116], uid);
    }
    ...
    pub fn set_gid(&mut self, gid: u64) {
        let _ = encode_numeric(&mut self.raw.bytes[116..124], gid);
    }
    ...
    pub fn set_mtime(&mut self, mtime: u64) {
        let _ = encode_numeric(&mut self.raw.bytes[136..148], mtime);
    }

    /// Set the entry type (byte 156).
    pub fn set_entry_type(&mut self, ty: EntryType) {
        self.raw.bytes[156] = ty.to_byte();
    }
    ...
    pub fn set_checksum(&mut self) {
        ...
        self.raw.bytes[148..156].fill(b' ');

        // Compute unsigned sum of all bytes
        let checksum: u64 = self.raw.bytes.iter().map(|&b| u64::from(b)).sum();

        // Write checksum as 7 octal digits + null (matches tar-rs format)
        let checksum_str = format!("{checksum:07o}\0");
        self.raw.bytes[148..156].copy_from_slice(checksum_str.as_bytes());
    }
    ...
    pub fn set_path(&mut self, path: &[u8]) -> Result<()> {
        if path.len() > 100 {
            return Err(HeaderError::InvalidOctal(path.to_vec()));
        }
        self.raw.bytes[0..100].fill(0);
        self.raw.bytes[0..path.len()].copy_from_slice(path);
        Ok(())
    }
    ...
    pub fn set_link_name(&mut self, link: &[u8]) -> Result<()> {
        if link.len() > 100 {
            return Err(HeaderError::InvalidOctal(link.to_vec()));
        }
        self.raw.bytes[157..257].fill(0);
        self.raw.bytes[157..157 + link.len()].copy_from_slice(link);
        Ok(())
    }
    ...
    pub fn set_username(&mut self, name: &[u8]) -> Result<()> {
        if name.len() > 32 {
            return Err(HeaderError::InvalidOctal(name.to_vec()));
        }
        self.raw.bytes[265..297].fill(0);
        self.raw.bytes[265..265 + name.len()].copy_from_slice(name);
        Ok(())
    }
    ...
    pub fn set_groupname(&mut self, name: &[u8]) -> Result<()> {
        ...
        self.raw.bytes[297..329].fill(0);
        self.raw.bytes[297..297 + name.len()].copy_from_slice(name);
        Ok(())
    }
    ...
    pub fn set_device(&mut self, major: u32, minor: u32) {
        let _ = encode_octal(&mut self.raw.bytes[329..337], u64::from(major));
        let _ = encode_octal(&mut self.raw.bytes[337..345], u64::from(minor));
    }
}
...
/// # Errors
///
/// Returns [`HeaderError::InvalidOctal`] if the value cannot be encoded
/// (only possible for values > u64::MAX, which can't happen).
pub fn encode_numeric(field: &mut [u8], value: u64) -> Result<()> {
    // Thresholds from tar-rs: use binary for large values
    let use_binary = if field.len() == 8 {
    ...
    if use_binary {
        // GNU base-256 encoding: high bit set, value in big-endian
        let len = field.len();
        field.fill(0);

        // Write the value in big-endian to the last 8 bytes (or fewer if field is smaller)
        let value_bytes = value.to_be_bytes();
        if len >= 8 {
            // Zero-fill leading bytes, then copy value
            field[len - 8..].copy_from_slice(&value_bytes);
        } else {
            // Field is smaller than 8 bytes, copy the least significant bytes
            field.copy_from_slice(&value_bytes[8 - len..]);
        }
...
pub fn encode_octal(field: &mut [u8], value: u64) -> Result<()> {
    if field.is_empty() {
...
mod tests {
    ...
    #[test]
    fn test_encode_numeric_octal() {
        // Small values should use octal encoding
        let mut field = [0u8; 12];
        encode_numeric(&mut field, 0o644).unwrap();
        assert_eq!(parse_numeric(&field).unwrap(), 0o644);

        // Zero
        encode_numeric(&mut field, 0).unwrap();
        assert_eq!(parse_numeric(&field).unwrap(), 0);

        // Max value that fits in 11 octal digits
        encode_numeric(&mut field, 0o77777777777).unwrap();
        assert_eq!(parse_numeric(&field).unwrap(), 0o77777777777);
    }

    #[test]
    fn test_encode_numeric_base256() {
        // Large values that need base-256 encoding
        let mut field = [0u8; 12];

        // Value >= 8GB (threshold for 12-byte fields)
        let large_value = 8589934592u64; // 8GB exactly
        encode_numeric(&mut field, large_value).unwrap();
        assert_eq!(field[0] & 0x80, 0x80, "should have base-256 marker");
        assert_eq!(parse_numeric(&field).unwrap(), large_value);

        // Even larger value
        let huge_value = 0x1234567890ABCDEFu64;
        encode_numeric(&mut field, huge_value).unwrap();
        assert_eq!(parse_numeric(&field).unwrap(), huge_value);
    }

    #[test]
    fn test_encode_numeric_uid_gid_threshold() {
        // 8-byte fields (uid/gid) use base-256 at a lower threshold
        let mut field = [0u8; 8];

        // Below threshold (2097152) - should use octal
        encode_numeric(&mut field, 2097151).unwrap();
        assert_eq!(field[0] & 0x80, 0, "should NOT have base-256 marker");
        assert_eq!(parse_numeric(&field).unwrap(), 2097151);

        // At threshold - should use base-256
        encode_numeric(&mut field, 2097152).unwrap();
        assert_eq!(field[0] & 0x80, 0x80, "should have base-256 marker");
        assert_eq!(parse_numeric(&field).unwrap(), 2097152);
    }

    #[test]
    fn test_header_new_old() {
        let header = Header::new_old();
        // Old format has no magic
        assert!(!header.is_ustar());
        assert!(!header.is_gnu());
    }

    #[test]
    fn test_header_try_as_ustar() {
        let ustar = Header::new_ustar();
        assert!(ustar.try_as_ustar().is_some());
        assert!(ustar.try_as_gnu().is_none());

        let gnu = Header::new_gnu();
        assert!(gnu.try_as_ustar().is_none());
        assert!(gnu.try_as_gnu().is_some());

        let old = Header::new_old();
        assert!(old.try_as_ustar().is_none());
        assert!(old.try_as_gnu().is_none());
    }
    ...
    #[test]
    fn test_header_setters_large_values() {
        let mut header = Header::new_ustar();

        // Large UID that needs base-256
        header.set_uid(0xFFFFFFFF);
        assert_eq!(header.uid().unwrap(), 0xFFFFFFFF);

        // Large size (>8GB)
        header.set_size(10_000_000_000);
        assert_eq!(header.entry_size().unwrap(), 10_000_000_000);
    }
    ...
    mod proptest_tests {
        ...
        mod builder_equivalence_tests {
            ...
            /// Build a header using tar-core's HeaderBuilder.
            fn build_with_tar_core(params: &FileParams, format: &str) -> [u8; 512] {
                let mut builder = match format {
                    "gnu" => HeaderBuilder::new_gnu(),
                    "ustar" => HeaderBuilder::new_ustar(),
            ...
            /// Build a header using tar-rs's Header API.
            fn build_with_tar_rs(params: &FileParams, format: &str) -> [u8; 512] {
            ...
            /// Compare two headers byte-by-byte, reporting differences.
            fn compare_header_bytes(
                our_bytes: &[u8; 512],
                tar_bytes: &[u8; 512],
            ) -> std::result::Result<(), TestCaseError> {
                if our_bytes == tar_bytes {
                    return Ok(());
                }

                // Find first difference for debugging
                for (i, (a, b)) in our_bytes.iter().zip(tar_bytes.iter()).enumerate() {
                    if a != b {
                        let field = match i {
                            0..=99 => "name",
                            100..=107 => "mode",
                            ...
                            329..=336 => "devmajor",
                            337..=344 => "devminor",
                            345..=499 => "prefix/gnu_fields",
                            500..=511 => "padding",
                            _ => "unknown",
                        };
                        return Err(TestCaseError::fail(format!(
                            "byte mismatch at offset {i} ({field}): ours=0x{a:02x} ({:?}), tar-rs=0x{b:02x} ({:?})",
                            *a as char, *b as char
                        )));
                    }
                }
            ...
            /// Build a symlink header using tar-core.
            fn build_symlink_with_tar_core(params: &SymlinkParams, format: &str) -> [u8; 512] {
                let mut builder = match format {
                    "gnu" => HeaderBuilder::new_gnu(),
                    "ustar" => HeaderBuilder::new_ustar(),
                    _ => panic!("unknown format"),
            ...
            /// Build a symlink header using tar-rs.
            fn build_symlink_with_tar_rs(params: &SymlinkParams, format: &str) -> [u8; 512] {
                let mut header = match format {
                    "gnu" => tar::Header::new_gnu(),
                    "ustar" => tar::Header::new_ustar(),
                    _ => panic!("unknown format"),
            ...
            /// Build a directory header using tar-core.
            fn build_dir_with_tar_core(params: &DirParams, format: &str) -> [u8; 512] {
                let mut builder = match format {
                    "gnu" => HeaderBuilder::new_gnu(),
                    "ustar" => HeaderBuilder::new_ustar(),
                    _ => panic!("unknown format"),
            ...
            /// Build a directory header using tar-rs.
            fn build_dir_with_tar_rs(params: &DirParams, format: &str) -> [u8; 512] {
                let mut header = match format {
                    "gnu" => tar::Header::new_gnu(),
                    "ustar" => tar::Header::new_ustar(),
                    _ => panic!("unknown format"),
            ...
            proptest! {
                ...
                /// Test UStar file headers are bit-identical.
                #[test]
                fn test_ustar_file_builder_equivalence(params in file_params_strategy()) {
                    let our_bytes = build_with_tar_core(&params, "ustar");
                    let tar_bytes = build_with_tar_rs(&params, "ustar");
                    compare_header_bytes(&our_bytes, &tar_bytes)?;
                }

                /// Test GNU file headers are bit-identical.
                #[test]
                fn test_gnu_file_builder_equivalence(params in file_params_strategy()) {
                    let our_bytes = build_with_tar_core(&params, "gnu");
                    let tar_bytes = build_with_tar_rs(&params, "gnu");
                    compare_header_bytes(&our_bytes, &tar_bytes)?;
                }

                /// Test UStar symlink headers are bit-identical.
                #[test]
                fn test_ustar_symlink_builder_equivalence(params in symlink_params_strategy()) {
                    let our_bytes = build_symlink_with_tar_core(&params, "ustar");
                    let tar_bytes = build_symlink_with_tar_rs(&params, "ustar");
                    compare_header_bytes(&our_bytes, &tar_bytes)?;
                }

                /// Test GNU symlink headers are bit-identical.
                #[test]
                fn test_gnu_symlink_builder_equivalence(params in symlink_params_strategy()) {
                    let our_bytes = build_symlink_with_tar_core(&params, "gnu");
                    let tar_bytes = build_symlink_with_tar_rs(&params, "gnu");
                    compare_header_bytes(&our_bytes, &tar_bytes)?;
                }

                /// Test UStar directory headers are bit-identical.
                #[test]
                fn test_ustar_dir_builder_equivalence(params in dir_params_strategy()) {
                    let our_bytes = build_dir_with_tar_core(&params, "ustar");
                    let tar_bytes = build_dir_with_tar_rs(&params, "ustar");
                    compare_header_bytes(&our_bytes, &tar_bytes)?;
                }

                /// Test GNU directory headers are bit-identical.
                #[test]
                fn test_gnu_dir_builder_equivalence(params in dir_params_strategy()) {
                    let our_bytes = build_dir_with_tar_core(&params, "gnu");
                    let tar_bytes = build_dir_with_tar_rs(&params, "gnu");
                    compare_header_bytes(&our_bytes, &tar_bytes)?;
                }
            }

            /// Test Header direct setters produce identical output to tar-rs.
            mod header_setters_equivalence {
                use super::*;

                /// Build using tar-core's Header::set_* methods directly.
                fn build_with_header_setters(params: &FileParams, format: &str) -> [u8; 512] {
                    let mut header = match format {
                        "gnu" => Header::new_gnu(),
                        "ustar" => Header::new_ustar(),
                        "old" => Header::new_old(),
                        _ => panic!("unknown format"),
                    };

                    header.set_path(params.path.as_bytes()).unwrap();
                    header.set_mode(params.mode);
                    header.set_uid(params.uid);
                    header.set_gid(params.gid);
                    header.set_size(params.size);
                    ...
                    header.set_username(params.username.as_bytes()).unwrap();
                    header.set_groupname(params.groupname.as_bytes()).unwrap();
                    header.set_checksum();
                ...
                proptest! {
                    ...
                    /// Test Header setters produce same output as tar-rs.
                    #[test]
                    fn test_header_setters_ustar_equivalence(params in file_params_strategy()) {
                        let our_bytes = build_with_header_setters(&params, "ustar");
                        let tar_bytes = build_with_tar_rs(&params, "ustar");
                        compare_header_bytes(&our_bytes, &tar_bytes)?;
                    }

                    /// Test Header setters produce same output as tar-rs (GNU).
                    #[test]
                    fn test_header_setters_gnu_equivalence(params in file_params_strategy()) {
                        let our_bytes = build_with_header_setters(&params, "gnu");
                        let tar_bytes = build_with_tar_rs(&params, "gnu");
                        compare_header_bytes(&our_bytes, &tar_bytes)?;
                    }
                }
            ...
            mod base256_equivalence {
                ...
                fn large_id_strategy() -> impl Strategy<Value = u64> {
                    // Values > 2097151 (0o7777777) require base-256 in 8-byte field
                    prop_oneof![
                ...
                /// Strategy for large file sizes that require base-256.
                /// (Reserved for future tests with large file sizes)
                #[allow(dead_code)]
                fn large_size_strategy() -> impl Strategy<Value = u64> {
                    // Values > 8GB (0o77777777777) require base-256 in 12-byte field
                    prop_oneof![
                        Just(8_589_934_592u64),                 // 8 GB, just over octal limit
                        Just(10_000_000_000u64),                // 10 GB
                        (8_589_934_592u64..100_000_000_000u64), // range
                    ]
                }

                /// Test large UID values are encoded identically.
                #[test]
                fn test_large_uid_encoding() {
                    // tar-rs uses base-256 for values that don't fit in octal
                    let large_uid = 2_500_000u64;

                    // Build with tar-rs
                    let mut tar_header = tar::Header::new_gnu();
                    tar_header.set_path("test.txt").unwrap();
                    tar_header.set_mode(0o644);
                    tar_header.set_uid(large_uid);
                    tar_header.set_gid(1000);
                    tar_header.set_size(0);
                    tar_header.set_mtime(0);
                    tar_header.set_entry_type(tar::EntryType::Regular);
                    tar_header.set_cksum();

                    // Build with tar-core
                    let mut our_header = Header::new_gnu();
                    our_header.set_path(b"test.txt").unwrap();
                    our_header.set_mode(0o644);
                    our_header.set_uid(large_uid);
                    our_header.set_gid(1000);
                    our_header.set_size(0);
                    our_header.set_mtime(0);
                    our_header.set_entry_type(EntryType::Regular);
                    our_header.set_checksum();

                    // Compare the uid field bytes (offset 108-116)
                    let our_bytes = our_header.as_bytes();
                    let tar_bytes = tar_header.as_bytes();

                    assert_eq!(
                        &our_bytes[108..116],
                        &tar_bytes[108..116],
                        "uid field mismatch for large value {large_uid}"
                    );

                    // Verify parsing works
                    assert_eq!(our_header.uid().unwrap(), large_uid);
                }

                /// Test large GID values are encoded identically.
                #[test]
                fn test_large_gid_encoding() {
                    let large_gid = 3_000_000u64;

                    let mut tar_header = tar::Header::new_gnu();
                    tar_header.set_path("test.txt").unwrap();
                    tar_header.set_mode(0o644);
                    tar_header.set_uid(1000);
                    tar_header.set_gid(large_gid);
                    tar_header.set_size(0);
                    tar_header.set_mtime(0);
                    tar_header.set_entry_type(tar::EntryType::Regular);
                    tar_header.set_cksum();
                    ...
                    our_header.set_path(b"test.txt").unwrap();
                    our_header.set_mode(0o644);
                    our_header.set_uid(1000);
                    our_header.set_gid(large_gid);
                    our_header.set_size(0);
                    our_header.set_mtime(0);
                    our_header.set_entry_type(EntryType::Regular);
                    our_header.set_checksum();
                    ...
                    assert_eq!(
                        &our_bytes[116..124],
                        &tar_bytes[116..124],
                        "gid field mismatch for large value {large_gid}"
                    );

                    assert_eq!(our_header.gid().unwrap(), large_gid);
                }

                proptest! {
                    #![proptest_config(ProptestConfig::with_cases(64))]

                    /// Test various large UID values.
                    #[test]
                    fn test_large_uid_proptest(uid in large_id_strategy()) {
                        let mut tar_header = tar::Header::new_gnu();
                        tar_header.set_path("test.txt").unwrap();
                        tar_header.set_mode(0o644);
                        tar_header.set_uid(uid);
                        tar_header.set_gid(1000);
                        tar_header.set_size(0);
                        tar_header.set_mtime(0);
                        tar_header.set_entry_type(tar::EntryType::Regular);
                        ...
                        prop_assert_eq!(
                            &our_header.as_bytes()[108..116],
                            &tar_header.as_bytes()[108..116],
                            "uid field mismatch for value {}", uid
                        );
                    }

                    /// Test various large GID values.
                    #[test]
                    fn test_large_gid_proptest(gid in large_id_strategy()) {
                        ...
                        prop_assert_eq!(
                            &our_header.as_bytes()[116..124],
                            &tar_header.as_bytes()[116..124],
                            "gid field mismatch for value {}", gid
                        );
                    }
                }
            }
            ...
            mod gnu_extensions_equivalence {
                ...
                /// Compare header sequences, ignoring data blocks.
                #[allow(dead_code)]
                fn compare_header_sequences(
                    our_headers: &[[u8; HEADER_SIZE]],
                    tar_headers: &[[u8; HEADER_SIZE]],
                ) -> std::result::Result<(), TestCaseError> {
                    prop_assert_eq!(
                        our_headers.len(),
                        tar_headers.len(),
                        "header count mismatch: ours={}, tar-rs={}",
                        our_headers.len(),
                        tar_headers.len()
                    );

                    for (i, (ours, theirs)) in
                        our_headers.iter().zip(tar_headers.iter()).enumerate()
                    {
                        if ours != theirs {
                            // Find first difference
                            for (j, (a, b)) in ours.iter().zip(theirs.iter()).enumerate() {
                                if a != b {
                                    return Err(TestCaseError::fail(format!(
                                        "header {} byte {} mismatch: ours=0x{:02x}, tar-rs=0x{:02x}",
                                        i, j, a, b
                                    )));
                                }
                            }
                ...
                fn test_gnu_longname_basic() {
                    // A path that's definitely > 100 bytes
                    ...
                    let tar_data = build_long_path_with_tar_rs(&params);
                    let tar_headers = extract_all_headers(&tar_data);

                    let our_headers = build_long_path_with_tar_core(&params);

                    // Both should have: LongName header + main header
                    // (tar-rs includes data block, we only emit headers)
                    assert!(
                        tar_headers.len() >= 2,
                        "tar-rs should have extension headers"
                    );
                    assert!(our_headers.len() >= 2, "we should have extension headers");

                    // Check that the first header is a GnuLongName type
                    let tar_ext = Header::from_bytes_exact(&tar_headers[0]);
                    let our_ext = Header::from_bytes_exact(&our_headers[0]);

                    assert_eq!(tar_ext.entry_type(), EntryType::GnuLongName);
                    assert_eq!(our_ext.entry_type(), EntryType::GnuLongName);

                    // The extension header name should be ././@LongLink
                    assert_eq!(tar_ext.path_bytes(), b"././@LongLink");
                    assert_eq!(our_ext.path_bytes(), b"././@LongLink");
                }

                #[test]
                fn test_gnu_longlink_basic() {
                    let long_target = "/very/long/target/".repeat(10);
                    ...
                    let tar_data = build_long_link_with_tar_rs(&params);
                    let tar_headers = extract_all_headers(&tar_data);

                    let our_headers = build_long_link_with_tar_core(&params);

                    // Both should have: LongLink header + main header
                    assert!(tar_headers.len() >= 2);
                    assert!(our_headers.len() >= 2);

                    // First header should be GnuLongLink
                    let tar_ext = Header::from_bytes_exact(&tar_headers[0]);
                    let our_ext = Header::from_bytes_exact(&our_headers[0]);

                    assert_eq!(tar_ext.entry_type(), EntryType::GnuLongLink);
                    assert_eq!(our_ext.entry_type(), EntryType::GnuLongLink);
                }

                proptest! {
                    #![proptest_config(ProptestConfig::with_cases(32))]

                    /// Test GNU long name headers are structurally equivalent with random metadata.
                    #[test]
                    fn test_gnu_longname_equivalence(params in long_path_file_params_strategy()) {
                        ...
                        // Verify both produce extension headers
                        prop_assert!(
                            tar_headers.len() >= 2,
                            "tar-rs should produce extension for path len {}",
                            params.path.len()
                        );
                        prop_assert!(
                            our_headers.len() >= 2,
                            "we should produce extension for path len {}",
                            params.path.len()
                        );

                        // Compare the extension header (first one)
                        let tar_ext = Header::from_bytes_exact(&tar_headers[0]);
                        let our_ext = Header::from_bytes_exact(&our_headers[0]);
                        ...
                        prop_assert_eq!(
                            ...
                            "extension size mismatch"
                        );

                        // Compare the main header (last one)
                        let tar_main = Header::from_bytes_exact(tar_headers.last().unwrap());
                        let our_main = Header::from_bytes_exact(our_headers.last().unwrap());

                        prop_assert_eq!(
                            tar_main.entry_type(),
                            our_main.entry_type(),
                            "main entry type mismatch"
                        );
                        prop_assert_eq!(
                    ...
                    /// Test GNU long link headers are structurally equivalent with random metadata.
                    #[test]
                    fn test_gnu_longlink_equivalence(params in long_link_params_strategy()) {
                        ...
                        prop_assert!(tar_headers.len() >= 2);
                        prop_assert!(our_headers.len() >= 2);

                        // Compare extension headers
                        let tar_ext = Header::from_bytes_exact(&tar_headers[0]);
                        let our_ext = Header::from_bytes_exact(&our_headers[0]);

                        prop_assert_eq!(tar_ext.entry_type(), EntryType::GnuLongLink);
                        prop_assert_eq!(our_ext.entry_type(), EntryType::GnuLongLink);
                        prop_assert_eq!(
                            tar_ext.entry_size().unwrap(),
                            our_ext.entry_size().unwrap(),
                            "link extension size mismatch"
                        );

                        // Compare main header metadata
                        let tar_main = Header::from_bytes_exact(tar_headers.last().unwrap());
                        let our_main = Header::from_bytes_exact(our_headers.last().unwrap());
