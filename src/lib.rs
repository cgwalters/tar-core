...
//!
//! All tar headers are 512 bytes. See [`OldHeader`], [`UstarHeader`], and
//! [`GnuHeader`] for the field-level documentation.
//!
//! # Example
//!
//! ```
...
/// are undefined in this format and may contain garbage.
...
///
/// The first 257 bytes (through `linkname`) share the same layout as
/// [`OldHeader`]; see that type for field-level documentation.
#[derive(Clone, Copy, FromBytes, IntoBytes, Immutable, KnownLayout)]
#[repr(C)]
pub struct UstarHeader {
    ...
    /// See [`OldHeader::cksum`].
    pub cksum: [u8; 8],
    /// Entry type flag (called `linkflag` in [`OldHeader`]).
    pub typeflag: [u8; 1],
    /// See [`OldHeader::linkname`].
    pub linkname: [u8; 100],
...
impl Default for UstarHeader {
    fn default() -> Self {
        // Start from the base Header (which defaults to UStar) and
        // reinterpret as the typed struct.
        *Self::ref_from_bytes(Header::default().as_bytes()).unwrap()
    }
}
...
/// times, and long name handling. The UStar `prefix` field is replaced with
/// additional metadata.
///
/// Fields through `dev_minor` share the same layout as [`UstarHeader`];
/// see [`OldHeader`] and [`UstarHeader`] for field-level documentation.
/// For sparse files, `size` is the on-disk content size (not the logical
/// file size); the logical size lives in [`realsize`](Self::realsize).
#[derive(Clone, Copy, FromBytes, IntoBytes, Immutable, KnownLayout)]
#[repr(C)]
pub struct GnuHeader {
    ...
    /// See [`OldHeader::gid`].
    pub gid: [u8; 8],
    /// See [`OldHeader::size`]. For sparse files this is the on-disk size.
    pub size: [u8; 12],
    /// See [`OldHeader::mtime`].
    pub mtime: [u8; 12],
    /// See [`OldHeader::cksum`].
    pub cksum: [u8; 8],
    /// See [`UstarHeader::typeflag`].
    pub typeflag: [u8; 1],
    /// See [`OldHeader::linkname`].
    pub linkname: [u8; 100],
    /// See [`UstarHeader::magic`] (`"ustar "` for GNU).
    pub magic: [u8; 6],
    /// See [`UstarHeader::version`] (`" \0"` for GNU).
    pub version: [u8; 2],
    /// See [`UstarHeader::uname`].
    pub uname: [u8; 32],
    /// See [`UstarHeader::gname`].
    pub gname: [u8; 32],
    /// See [`UstarHeader::dev_major`].
    pub dev_major: [u8; 8],
    /// See [`UstarHeader::dev_minor`].
    pub dev_minor: [u8; 8],
...
impl Default for GnuHeader {
    fn default() -> Self {
        // Start from the base Header and reinterpret, then set GNU
        // magic/version over the UStar defaults.
        let mut gnu = *Self::ref_from_bytes(Header::default().as_bytes()).unwrap();
...
impl Header {
    ...
    pub fn entry_type(&self) -> EntryType {
        EntryType::from_byte(self.as_old().linkflag[0])
    }
    ...
    pub fn entry_size(&self) -> Result<u64> {
        parse_numeric(&self.as_old().size)
    }
    ...
    pub fn mode(&self) -> Result<u32> {
        parse_numeric(&self.as_old().mode).map(|v| v as u32)
    }
    ...
    pub fn mtime(&self) -> Result<u64> {
        parse_numeric(&self.as_old().mtime)
    }
    ...
    pub fn path_bytes(&self) -> &[u8] {
        truncate_null(&self.as_old().name)
    }

    /// Get the raw link name bytes.
    #[must_use]
    pub fn link_name_bytes(&self) -> &[u8] {
        truncate_null(&self.as_old().linkname)
    }
    ...
    pub fn verify_checksum(&self) -> Result<()> {
        let expected = parse_octal(&self.as_old().cksum)?;
    ...
    pub fn set_entry_type(&mut self, ty: EntryType) {
        self.as_old_mut().linkflag[0] = ty.to_byte();
    }
    ...
    pub fn set_path(&mut self, path: &[u8]) -> Result<()> {
        ...
        let name = &mut self.as_old_mut().name;
        name.fill(0);
...
mod tests {
    ...
    fn test_path_bytes() {
        let mut header = Header::new_ustar();
        header.set_path(b"hello").unwrap();
    ...
    fn test_parse_numeric_base256_in_header() {
        ...
        // Write a base-256 encoded value directly into the size field.
        // 12-byte field: first byte has 0x80 marker, remaining 11 bytes are big-endian.
        let size_field = &mut header.as_ustar_mut().size;
        size_field.fill(0);
    ...
    fn test_parse_numeric_base256_uid_gid() {
        ...
        // Write a base-256 encoded value directly into the uid field.
        let uid_field = &mut header.as_ustar_mut().uid;
        uid_field.fill(0);
        uid_field[0] = 0x80; // base-256 marker
        uid_field[7] = 0x42; // value = 66
        assert_eq!(header.uid().unwrap(), 66);

        // Write a base-256 encoded value directly into the gid field.
        let gid_field = &mut header.as_ustar_mut().gid;
        gid_field.fill(0);
    ...
    fn test_from_bytes() {
        let mut header = Header::new_ustar();
        header.set_path(b"test").unwrap();

        let header = Header::from_bytes(header.as_bytes());
        assert!(header.is_ustar());
    ...
    fn test_from_bytes_gnu() {
        ...
        let header = Header::from_bytes(gnu.as_bytes());
        assert!(header.is_gnu());
        assert!(!header.is_ustar());
    }
    ...
    fn test_path_exactly_100_bytes() {
        ...
        header.set_path(path.as_bytes()).unwrap();
    ...
    fn test_link_name_exactly_100_bytes() {
        ...
        header.set_link_name(target.as_bytes()).unwrap();
    ...
    fn test_gnu_atime_ctime() {
        ...
        // Set atime via the typed field
        header.as_gnu_mut().atime.copy_from_slice(b"12345670123\0");
        let gnu = header.as_gnu();
    ...
    mod proptest_tests {
        ...
        mod format_detection_tests {
            ...
            fn test_old_format_detection() {
                // Create an old-format header using typed constructors/setters
                let mut header = Header::new_old();
                header.set_path(b"test").unwrap();
                header.set_mode(0o644).unwrap();
                header.set_size(0).unwrap();
