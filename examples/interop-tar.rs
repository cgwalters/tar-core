...
fn tar_list(archive_path: &Path) -> Vec<Vec<u8>> {
...
/// Uses `--quoting-style=literal` to avoid octal escaping of non-ASCII bytes.
fn tar_verbose_list(archive_path: &Path) -> Vec<u8> {
    let output = Command::new("tar")
        .arg("--quoting-style=literal")
        .arg("-tvf")
        .arg(archive_path)
...
fn tar_extract(archive_path: &Path, dest_dir: &Path) {
    let output = Command::new("tar")
        .arg("xf")
        .arg(archive_path)
        .arg("-C")
...
fn tar_create(archive_path: &Path, format: &str, src_dir: &Path, filenames: &[&[u8]]) {
    ...
    for name in filenames {
        cmd.arg(OsStr::from_bytes(name));
    }

    let output = cmd.output().expect("failed to run tar cf");
    assert!(
        output.status.success(),
        "tar cf failed: {}",
        std::str::from_utf8(&output.stderr).unwrap_or("<non-utf8>")
...
fn test_tar_core_to_gnu_tar(entries: &[EntryParams], format: &str) {
    ...
    let listed = tar_list(&archive_path);
    assert_eq!(
        entries.len(),
        listed.len(),
    ...
    let verbose = tar_verbose_list(&archive_path);
    for entry in entries {
    ...
    tar_extract(&archive_path, extract_dir.path());
...
fn test_gnu_tar_to_tar_core(entries: &[EntryParams], format: &str) {
    ...
    tar_create(&archive_path, format, src_dir.path(), &filenames);
...
fn smoke_test_basic_roundtrip() {
    ...
    for format in &["gnu", "pax"] {
        test_tar_core_to_gnu_tar(&entries, format);
    }
}

fn smoke_test_directories() {
    let entries = vec![
        EntryParams {
            path: b"mydir".to_vec(),
            mode: 0o755,
...
fn smoke_test_non_utf8_gnu_format() {
    ...
    test_tar_core_to_gnu_tar(&entries, "gnu");
}

fn smoke_test_gnu_tar_to_tar_core() {
    let entries = vec![
        EntryParams {
    ...
    for format in &["gnu", "posix"] {
        test_gnu_tar_to_tar_core(&entries, format);
    }
}

fn smoke_test_gnu_tar_non_utf8_roundtrip() {
...
fn smoke_test_gnu_tar_directories() {
    let entries = vec![EntryParams {
...
fn main() {
    ...
    run_random_tests("random_tar_core_to_gnu_tar_gnu", 16, |entries| {
        ...
        if !unique.is_empty() {
            test_tar_core_to_gnu_tar(&unique, "gnu");
        }
    });
    ...
    run_random_tests("random_tar_core_to_gnu_tar_pax", 16, |entries| {
        ...
        if !unique.is_empty() {
            test_tar_core_to_gnu_tar(&unique, "pax");
        }
    });
    ...
    run_random_tests("random_gnu_tar_to_tar_core", 16, |entries| {
        ...
        if !unique.is_empty() {
            test_gnu_tar_to_tar_core(&unique, "gnu");
        }
    });
